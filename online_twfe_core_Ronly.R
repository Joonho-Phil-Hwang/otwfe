...
otwfe <- function(data,
  ...
  if (nrow(df_stream) == 0L) {
    if (verbose) cat("추가 스트리밍 데이터 없음. 초기 상태 반환.\n")
    return(structure(
  ...
  if (verbose) {
    cat(sprintf("스트리밍 처리: %s개 unit, %s개 관측치\n",
                format(n_stream, big.mark = ","),
                format(nrow(df_stream), big.mark = ",")))
    pb <- make_progress_bar(0, n_stream)
  }

  for (u_idx in seq_along(stream_unit_chs)) {
    uid_ch  <- stream_unit_chs[u_idx]
    ...
    if (!uid_ch %in% names(units)) {
      # -------------------------------------------------------
      # 케이스 A: 새 unit → Algorithm 1
      # warm-up에서 T_support = T_max를 이미 확보했으므로
      # 스트리밍 unit은 기존 T_support 내의 관측치만 처리
      # -------------------------------------------------------
      if (any(times_u > S_pre$T_support)) {
        warning(sprintf(
          "Unit '%s'가 T_support(%d)를 초과하는 calendar time을 포함합니다. " ,
      ...
      # track_all_units = TRUE일 때만 per-unit state 저장
      # FALSE(기본값)이면 저장 생략 → units 리스트에 warm-up unit만 유지
      if (track_all_units) {
      ...
      # -------------------------------------------------------
      # 케이스 B / C: 기존 unit (= warm-up unit) → Algorithm 2 or 3
      # -------------------------------------------------------
