...
#'
#' The file must be sorted by \code{id_col} (non-decreasing). This is required
#' for chunk-based unit-boundary detection and is a prerequisite for the
#' streaming algorithm. If your file is sorted by time (e.g., year-ordered
#' panel data), pre-sort it once with \code{\link{sort_panel_csv}()}.
#'
...
#' @seealso \code{\link{sort_panel_csv}} for pre-sorting unsorted CSV files
#'
otwfe_file <- function(path,
  ...
  repeat {
    ...
    # Sort order check: last id in carry_over must not exceed first id in new chunk
    if (!is.null(carry_over) && nrow(carry_over) > 0L &&
        carry_over[[id_col]][nrow(carry_over)] > raw[[id_col]][1L])
      stop(sprintf(paste0(
        "Sort order violation detected at chunk %d.\n",
        "  otwfe_file() requires the CSV to be sorted by '%s'.\n",
        "  Pre-sort once with: sort_panel_csv('%s', id_col='%s', output_path='...')"),
        chunk_idx, id_col, basename(path), id_col))
